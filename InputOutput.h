...
public:
	static std::vector<std::vector<long double>>calculateDistance(std::vector<std::pair<long double, long double>>& vect)
	{
		...
		{
			//distante[ii].resize(vect.size());
			for (auto jj = 0; jj < vect.size(); jj++)
			{
				distante[ii][jj] = sqrt((vect[jj].first - vect[ii].first) * (vect[jj].first - vect[ii].first) +
										(vect[jj].second - vect[ii].second) * (vect[jj].second - vect[ii].second));
				if (ii == jj)
